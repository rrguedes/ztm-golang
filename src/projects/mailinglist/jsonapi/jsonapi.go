...
func GetEmail(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if _, err := mdb.GetEmail(db, entry.Email); err != nil {
			returnErr(w, err, 400)
			return
		}
...
func GetEmailBatch(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		queryOptions := mdb.EmailBatchQueryParams{}
		fromJson(req.Body, &queryOptions)

		if queryOptions.Count <= 0 || queryOptions.Page <= 0 {
			returnErr(w, errors.New("page and count parameters are required and must be greater than 0"), 400)
			return
		}
...
func Serve(db *sql.DB, bind string) {
	...
	if err != nil {
		log.Fatalf("JSON fatal server error: %v", err)
	}
